...
function getLastmod(filename) {
  ...
  if (filename.match(/^(fridge-repair|washer-repair|dryer-repair|dishwasher-repair|oven-repair|stove-repair)-near-me\.html$/)) return '2025-11-03';
  if (filename.match(/-near-me\.html$/)) return '2025-11-25';
  ...
  if (filename.match(/^(about|contact|book|pricing|services|areas|brands)\.html$/)) return '2025-11-25';
  return '2026-01-15';
}

function getPriority(filename) {
  if (filename === 'index.html') return '1.0';
  if (filename.match(/-near-me\.html$/) && !filename.includes('-near-me-')) return '0.9';
  if (filename.match(/-(toronto|mississauga|brampton)\.html$/)) return '0.8';
  if (filename.match(/^blog\//)) return '0.7';
  if (filename.match(/^(about|book|areas)\.html$/)) return '0.6';
  return '0.6';
}
...
console.log(`Sitemap generated: ${urls.length} URLs → appliancerepairneary.com/sitemap.xml`);
